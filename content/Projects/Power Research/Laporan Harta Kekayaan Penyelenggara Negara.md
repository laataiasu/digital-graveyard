---
aliases:
  - LHKPN
  - Asset Declaration
tags:
  - indonesia
  - law
  - compliance
  - anti-corruption
  - lhkpn
  - kpk
  - public-official
date: 2025-07-13
source: KPK Regulation No. 7/2016
publish_external: true
---

The **LHKPN** (Laporan Harta Kekayaan Penyelenggara Negara) is an **asset declaration report** submitted by public officials in **Indonesia**. It is managed by the **Komisi Pemberantasan Korupsi (KPK)**, the country’s Corruption Eradication Commission.

In terms of **entity type**, **LHKPN** itself is not a legal entity or organization — it's a **reporting system or instrument**.

To clarify the **entity type** in different contexts:

- **Legal or organizational context**: LHKPN is **not** a legal entity. It is a **compliance/reporting mechanism**.
    
- **Data classification context** (e.g., in databases or knowledge graphs): LHKPN may be classified as a **document type** or **regulatory compliance form**.
    
- **Governmental function context**: It falls under **public governance**, **transparency**, and **anti-corruption instruments**.
    

Let me know what specific context you're asking about (legal, data modeling, ontology, etc.), and I can tailor the answer further.


#lhkpn #kpk #compliance #anti-corruption #indonesia #public-official